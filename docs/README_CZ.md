# Dokumentace Sticky Notes 1.0.0

[Česky](README_CZ.md) | [English](README.md) | [← Hlavní README](../README_CZ.md)

Tato dokumentace popisuje funkce vydání **Sticky Notes 1.0.0**.

## Uživatelská dokumentace

- [Uživatelská příručka](USER_GUIDE_CZ.md) – základní práce s aplikací, lístečky, filtry a úkoly.
- [Editor lístečků](EDITOR_CZ.md) – formátování obsahu a práce s rich-text editorem.
- [Kategorie a vzhled](CATEGORIES_CZ.md) – osobní a systémové kategorie, barvy, ikony a styly.
- [Sdílení, přiřazení a notifikace](SHARING_CZ.md) – uživatelé, skupiny a dokončování úkolů.
- [Dashboard widget](DASHBOARD_CZ.md) – kompaktní zobrazení na Dashboardu.
- [Nastavení aplikace](SETTINGS_CZ.md) – widget, stránkování, layout a vzhled.
- [Notifikace](NOTIFICATIONS_CZ.md) – současné notifikace v 1.0.0 a plánovaný model Nextcloud + ntfy pro 1.1.0.

## Plán další verze

- [Roadmap 1.1.0](ROADMAP_CZ.md) – mobilní rozhraní, O aplikaci, ntfy a příprava repozitáře pro oficiální distribuci.

## Instalace a údržba

- [Instalace](INSTALL_CZ.md)
- [Aktualizace](UPDATE_CZ.md)

## Pro vývojáře

- [Struktura projektu](DEVELOPMENT.md)
