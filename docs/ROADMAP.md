...
This document describes **planned changes for version 1.1.0**. These features are not part of the released 1.0.0 version.
...
In 1.0.0 the new-note editor is usable on mobile, but the main page, overview areas, and some settings can be clipped or unable to scroll correctly.

Planned work includes a one-column note board on narrow screens, viewport-safe widths, responsive toolbars/filters/statistics/pagination, mobile-safe settings and category management, responsive sharing/assignment/edit dialogs, and verification on phone, tablet, and desktop. The already working mobile editor must not regress.
...
Built-in Nextcloud notifications will remain available. Version 1.1.0 is planned to add an optional per-user **ntfy** delivery channel.
...
Planned events include direct assignment, group assignment, sharing, completion of a task created by the user, reopening, approaching deadline, and deadline reached/overdue. Group notifications should respect each member's own preferences. A test-notification button is planned.
...
Sticky Notes 1.0.0 and the initial 1.1.0 development target **Nextcloud 34**. Nextcloud 35 support will be declared only after the project itself moves to Nextcloud 35 and the app is tested there. The older major will then no longer be actively tested.

## 5. Repository and distribution

The development repository may keep `src/`, `docs/`, `release/`, and `old/`. A planned `build-release.sh` will create a clean Nextcloud package whose top-level directory is `stickynotes/` and contains only runtime application content.
...
This roadmap belongs to the next development cycle. Planned items should move into normal user documentation only after they are implemented and tested.
